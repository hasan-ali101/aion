  ...
  return (
    <Card
      ...
      className={cn(
        ...
        "duration-400 h-fit w-full min-w-64 max-w-96 flex-1 transition-colors ease-out md:duration-200",
      )}
    >
      <CardHeader className="flex-row justify-between gap-2">
        ...
        >
          <ArrowDown
            className={cn(
              expanded && "rotate-180",
              "duration-400 h-4 w-4 transition-transform ease-out",
            )}
          />
        </div>
      </CardHeader>
      <CardContent className="relativ">
        <Image
          alt="alt"
          width={1200}
          height={1000}
          src={backgroundImage}
          className={cn(
            expanded ? "absolute object-cover opacity-5" : "hidden lg:flex",
        ...
        <div
          className={cn(
            expanded
              ? "h-full max-h-[500px] overflow-clip pb-6"
              : "h-0 max-h-0 overflow-clip lg:h-full lg:max-h-full",
            "md:duration-400 relative text-white transition-all ease-out",
          )}
        >
